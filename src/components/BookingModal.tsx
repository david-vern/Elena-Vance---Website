          ...
          >
            ...
            <div className="flex items-center justify-between border-b border-[#EAE3D8] px-6 py-4 bg-white">
              <div>
                ...
                <p className="text-xs text-[#7A8B72] mt-0.5">Complimentary 30-Minute Alignment Consultation</p>
              </div>
              <button
                onClick={onClose}
            ...
            <div className="p-6 md:p-8 max-h-[75vh] overflow-y-auto">
              {step === 1 && (
                <div className="space-y-6">
                  <div>
                    <span className="text-xs font-mono text-[#C8A96B] tracking-widest uppercase">Step 01 / 03</span>
                    <h4 className="font-serif text-2xl text-[#2D2D2D] mt-1 font-medium">Select Focus Alignment</h4>
                    <p className="text-[#2D2D2D]/70 text-sm mt-1">Which area of your life would benefit most from our exploration alignment?</p>
                  </div>
                    ...
                    {SERVICES.map(service => (
                      ...
                      >
                        <div className="flex items-start justify-between">
                          <h5 className="font-serif font-medium text-sm text-[#2D2D2D]">{service.title}</h5>
                        ...
                        <p className="text-xs text-[#2D2D2D]/60 mt-1 lines-clamp-2 leading-relaxed">{service.description}</p>
                      </button>
                    ))}
                  </div>
              ...
              {step === 2 && (
                <div className="space-y-6">
                  <div>
                    ...
                    <p className="text-[#2D2D2D]/70 text-sm mt-1">Select your preferred date and local time slot.</p>
                  </div>
                    ...
                    <div className="md:col-span-5 space-y-3">
                      ...
                      {formData.date ? (
                        ...
                        <div className="h-[200px] flex items-center justify-center border border-dashed border-[#EAE3D8] rounded-xl bg-white p-4 text-center">
                          <p className="text-xs text-[#2D2D2D]/50 leading-relaxed">Please select a folder date on the calendar to reveal active meeting windows.</p>
                        </div>
                      )}
                    </div>
                  </div>
              ...
              {step === 3 && (
                <form onSubmit={handleComplete} className="space-y-6">
                  <div>
                    ...
                    <p className="text-[#2D2D2D]/70 text-sm mt-1">Please provide details so Elena may review your goals prior to dialing.</p>
                  </div>

                  <div className="space-y-4">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                        <label className="block text-xs font-medium text-[#2D2D2D] mb-1.5 flex items-center gap-1.5">
                  ...
                  {/* Summary Ribbon */}
                  <div className="bg-[#EAE3D8]/40 border border-[#EAE3D8] p-4 rounded-xl flex items-center justify-between text-xs">
                    <div className="space-y-1 text-[#2D2D2D]/80">
                      <p><strong>Selected Program:</strong> {SERVICES.find(s => s.id === formData.serviceId)?.title}</p>
                      <p className="flex items-center gap-1">
                        <Calendar className="h-3 w-3" /> {formData.date} at <Clock className="h-3 w-3 inline ml-1" /> {formData.timeSlot}
                      </p>
                    </div>
                  ...
                  <div className="flex justify-between pt-4 border-t border-[#EAE3D8]">
                    ...
                    <button
                      type="submit"
                      className="bg-[#7A8B72] text-white hover:bg-[#687860] px-7 py-3 rounded-xl font-medium text-sm shadow-md transition-all flex items-center gap-2 cursor-pointer"
              ...
              {step === 4 && (
                <div className="py-6 text-center space-y-6">
                  ...
                  <div className="space-y-2">
                    ...
                    <p className="text-[#2D2D2D]/70 text-sm max-w-md mx-auto leading-relaxed">
                      Thank you, {formData.name}. Elena has received your request and look forward to meeting with you.
                    </p>
                  </div>

                  <div className="max-w-md mx-auto bg-white border border-[#EAE3D8] rounded-xl p-5 text-left text-xs space-y-3 shadow-sm">
                    <p className="font-serif font-semibold text-sm text-[#2D2D2D] border-b border-[#F8F5F0] pb-2">Preparation Guidance</p>
                    <div className="space-y-2 text-[#2D2D2D]/80 leading-relaxed">
