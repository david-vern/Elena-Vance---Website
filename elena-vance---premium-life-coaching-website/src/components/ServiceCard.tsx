    ...
    >
      <div className="space-y-4">
        ...
        <div className="space-y-2">
          ...
          <p className="text-xs text-[#2D2D2D]/70 leading-relaxed min-h-[50px]">{service.description}</p>
        </div>

        {/* Dynamic Accordion expand for full program description */}
        <button
          onClick={() => setIsExpanded(!isExpanded)}
          className="text-xs font-semibold tracking-wide text-[#7A8B72] flex items-center gap-1.5 focus:outline-none hover:text-[#687860] cursor-pointer"
        >
          ...
          {isExpanded && (
            ...
            >
              <div className="space-y-1.5">
                ...
                <p className="text-xs text-[#2D2D2D]/80 leading-relaxed">{service.longDescription}</p>
              </div>

              <div className="space-y-2">
                <p className="text-[10px] uppercase font-mono text-[#C8A96B] tracking-wider">Included Alignments</p>
                <ul className="space-y-1.5 text-xs text-[#2D2D2D]/70">
              ...
              <div className="space-y-1 bg-[#F8F5F0]/60 p-3 rounded-lg border border-[#EAE3D8]/50 text-[11px] leading-relaxed">
                <p className="text-[#2D2D2D]/50 font-medium">✨ Target Audience:</p>
                <p className="text-[#2D2D2D]/80">{service.suitedFor}</p>
              </div>
            </motion.div>
          )}
        </AnimatePresence>
